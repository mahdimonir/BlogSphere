...
export default function tabActions({ tabs, itemInfo }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_URL}/${currentTabAPI.api}`);
        console.log("URL:", `${API_URL}/${currentTabAPI.api}`);
    ...
    <>
      <div className="flex flex-col  flex-1  bg-white p-3 rounded-md">
        <div className="flex gap-3 text-gray-700">
          {tabs.map((item, i) => (
            <div
              ...
              className="cursor-pointer hover:bg-gray-300 px-3 py-1 text-sm"
            >
              {item.content}{" "}
            </div>
          ))}
        ...
        <div className="w-full flex-1 h-full mx-3  flex justify-center items-center  ">
          ...
          {err && (
            <div className="w-full h-full flex justify-center items-center text-red-500 font-bold ">
              Something went wrong ! Need to set up proper api{" "}
            </div>
          )}
          {!loading && !err && !tabData ? (
            <div className="p-3"> {content}</div>
          ) : (
            <div className="w-full h-full flex justify-center items-center text-sm font-bold">
              Empty
            </div>
          )}
        </div>
...
const PendingPosts = ({ data }) => {
  return (
    <>
      <div>List of all pending posts</div>
    </>
  );
};
...
const Suspended = ({ data }) => {
  return (
    <>
      <div>List of all suspended posts</div>
    </>
  );
};
const Action = ({ data }) => {
